...
describe('Calculator', () => {
  ...
  describe('Error handling', () => {
    ...
    test('should handle unexpected end after operator', () => {
      const result = calculator.calculate(['2', '+']);
      expect(result.error).toBe('Unexpected end of expression');
      expect(result.result).toBeUndefined();
    });

    test('should handle unexpected end after multiplication', () => {
      const result = calculator.calculate(['2', '*']);
      expect(result.error).toBe('Unexpected end of expression');
      expect(result.result).toBeUndefined();
    });

    test('should handle unexpected end after division', () => {
      const result = calculator.calculate(['2', '/']);
      expect(result.error).toBe('Unexpected end of expression');
      expect(result.result).toBeUndefined();
    });

    test('should handle unexpected end after unary minus', () => {
      const result = calculator.calculate(['-']);
      expect(result.error).toBe('Unexpected end of expression');
      expect(result.result).toBeUndefined();
    });

    test('should handle unexpected end after unary plus', () => {
      const result = calculator.calculate(['+']);
      expect(result.error).toBe('Unexpected end of expression');
      expect(result.result).toBeUndefined();
    });

    test('should handle invalid multiplication sequence', () => {
      const result = calculator.calculate(['2', '*', '*', '3']);
      expect(result.error).toBe('Invalid token: *');
      expect(result.result).toBeUndefined();
    });

    test('should handle invalid division sequence', () => {
      const result = calculator.calculate(['2', '/', '/', '3']);
      expect(result.error).toBe('Invalid token: /');
      expect(result.result).toBeUndefined();
    });
